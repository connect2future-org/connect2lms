...
type IntegrityNotice = { eventType: IntegrityEventType; count: number; autoSubmitted: boolean };

export function IntegrityNoticeBanner({ notice, threshold }: { notice: IntegrityNotice; threshold: number }) {
  return <div role="alert" aria-live="assertive" className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 text-sm ${notice.autoSubmitted ? "border-rose-300/60 bg-rose-300/15 text-rose-100" : "border-amber-300/45 bg-amber-300/10 text-amber-100"}`}><span className="flex items-center gap-3"><AlertTriangle className="size-5 shrink-0 text-amber-300" /><span><strong>{notice.autoSubmitted ? "Assessment auto-submitted" : "Integrity warning"}</strong><span className="ml-2">{integrityWarningMessage(notice.eventType, notice.count, notice.autoSubmitted, threshold)}</span></span></span><span className="font-mono text-xs">{notice.count}/{threshold}</span></div>;
}
...
function TakeAssessmentContent() {
  ...
  if (completed) return <main className="blueprint-surface flex min-h-screen items-center justify-center p-6"><section className="blueprint-panel w-full max-w-xl p-8 text-center"><CheckCircle2 className="mx-auto size-12 text-emerald-300" /><p className="eyebrow mt-5">ASSESSMENT RECORDED</p><h1 className="mt-2 text-3xl font-semibold text-white">{completed.status === "AUTO_SUBMITTED" ? "Attempt automatically submitted" : "Assessment submitted"}</h1>{integrityNotice && <div className="mt-6 text-left"><IntegrityNoticeBanner notice={integrityNotice} threshold={policy.violationThreshold ?? 5} /></div>}{completed.score !== null && completed.percentage !== null ? (<div className="mt-7 grid grid-cols-2 gap-3"><div className="metric-card"><span>Score</span><strong>{completed.score}</strong></div><div className="metric-card"><span>Percentage</span><strong>{completed.percentage}%</strong></div></div>) : (<div className="mt-7 rounded-xl border border-amber-300/30 bg-amber-300/10 p-5"><p className="text-sm font-semibold text-amber-100">Your response has been recorded successfully.</p><p className="mt-2 text-xs text-amber-100/70">Results will be available once your teacher publishes the scores. Check back in your results history.</p></div>)}<Button className="mt-7 bg-cyan-300 text-[#05205c] hover:bg-cyan-200" onClick={() => navigate("/student")}>View assessment history</Button></section></main>;
