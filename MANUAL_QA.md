...
## Teacher publishing repair and Students directory

The reported issue was traced to the Teacher dashboard publishing UI and server contract only offering confirmed-roster targets. The dashboard now offers two explicit choices: **All active students** and **Selected active students**. Both manually provisioned and confirmed-imported student profiles are eligible when active and inside the current Teacher and institution scope. The server validates the selected set before changing the assessment to `PUBLISHED`; duplicates are normalized, inactive/out-of-scope students are rejected, and the assignment mutation follows the same active-scope rule.

A dedicated `/teacher/students` page is now registered and linked from the Teacher sidebar as **Students**. It displays the roster-aligned columns Name, Email, Username, Student ID, USN, Branch, Semester, Section, Class, source, and status, with search and directory export. It uses the existing Teacher-scoped `people.teacher.listStudents` procedure and never exposes password hashes or data outside the Teacher’s institution scope.

Anonymous desktop screenshots of `/teacher` and `/teacher/students` continue to resolve to the intended **SESSION REQUIRED** boundary. Automated validation after the repair passed TypeScript, 21 test files with 65 passing assertions and 3 skipped live-Atlas assertions, and the production build. Authenticated acceptance remains required for confirming that manually entered students appear in the publish selector and that the new sidebar page renders live records.
