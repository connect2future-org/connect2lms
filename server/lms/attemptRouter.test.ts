...
import { policyAllowsEvent } from "./attemptRouter";
...
describe("authoritative attempt policy", () => {
  ...
  it("maps browser events to the assessment policy on the server", () => { const policy = { detectTabSwitch: false, detectClipboard: true, detectShortcuts: false }; expect(policyAllowsEvent(policy, "TAB_HIDDEN")).toBe(false); expect(policyAllowsEvent(policy, "COPY")).toBe(true); expect(policyAllowsEvent(policy, "SHORTCUT")).toBe(false); });
});
