...
const mocks = vi.hoisted(() => ({ getCredentialUser: vi.fn(), getDb: vi.fn(), getUserById: vi.fn(), updateOne: vi.fn(), writeAudit: vi.fn(), compare: vi.fn() }));
vi.mock("../db", () => ({ getCredentialUser: mocks.getCredentialUser, getDb: mocks.getDb, getUserById: mocks.getUserById, publicUser: (user: LmsUser) => ({ id: user.id, name: user.name, role: user.role, schoolId: user.schoolId, username: user.username }) }));
