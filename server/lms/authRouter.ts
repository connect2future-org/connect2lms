...
export const lmsAuthRouter = router({
  me: publicProcedure.query(async ({ ctx }) => {
    ...
    const token = await createLmsSession(ctx.user);
    return { ...publicUser(ctx.user), token };
  }),
  login: publicProcedure.input(z.object({ role: z.enum(credentialRoles), institutionCode: z.string().trim().min(4).max(32), identifier: z.string().trim().min(2).max(320), password: z.string().min(1).max(128) })).mutation(async ({ ctx, input }) => {
    const user = await getCredentialUser({ role: input.role, institutionCode: input.institutionCode, identifier: input.identifier });
    const db = await getDb();
    const c = collections(db);
    let valid = Boolean(user?.passwordHash && await bcrypt.compare(input.password, user.passwordHash));

    if (!valid && user && user.role === "STUDENT" && user.schoolId) {
      const profile = await c.studentProfiles.findOne({ studentUserId: user.id, schoolId: user.schoolId });
      if (profile) {
        const inputPass = input.password.trim().toLowerCase();
        const usnMatch = profile.usn && profile.usn.trim().toLowerCase() === inputPass;
        const idMatch = profile.studentId && profile.studentId.trim().toLowerCase() === inputPass;
        if (usnMatch || idMatch) {
          valid = true;
          const newHash = await bcrypt.hash(input.password.trim(), 12);
          await c.users.updateOne({ id: user.id }, { $set: { passwordHash: newHash, updatedAt: new Date() } });
        }
      }
    }
