...
export const lmsAuthRouter = router({
  ...
  login: publicProcedure.input(z.object({ role: z.enum(credentialRoles), institutionCode: z.string().trim().min(4).max(32), identifier: z.string().trim().min(2).max(320), password: z.string().min(8).max(128) })).mutation(async ({ ctx, input }) => {
    ...
    const db = await getDb(); await collections(db).users.updateOne({ id: user.id, schoolId: user.schoolId }, { $set: { lastLogin: new Date(), lastSignedIn: new Date(), updatedAt: new Date() } }); const token = await createLmsSession(user); ctx.res.cookie(LMS_SESSION_COOKIE, token, lmsSessionCookieOptions); await writeAudit(db, { actorId: user.id, actorRole: user.role, schoolId: user.schoolId, action: "AUTH_LOGIN", targetType: "USER", targetId: user.id, ...requestAuditContext(ctx.req) }); return { success: true, message: "Signed in successfully.", data: { user: publicUser(user) } };
  }),
  logout: publicProcedure.mutation(({ ctx }) => { ctx.res.clearCookie(LMS_SESSION_COOKIE, { ...lmsSessionCookieOptions, maxAge: -1 }); return { success: true, message: "Signed out successfully.", data: null }; }),
});
