...
import { columnMapping, normalizeImportRows, summarizeImportRows, type ImportRow } from "./importUtils";
...
export const importRouter = router({
  preview: protectedProcedure.input(z.object({ sourceName: z.string().trim().min(1).max(255), rows: z.array(rawRow).min(1).max(1000) })).mutation(async ({ ctx, input }) => {
    const actor = requireRole(ctx.user, ["TEACHER"]); const db = await getDb(); const c = collections(db); const normalized = normalizeImportRows(input.rows);
  ...
  confirm: protectedProcedure.input(z.object({ batchId: z.number().int().positive(), defaultTemporaryPassword: z.string().min(12).max(128) })).mutation(async ({ ctx, input }) => {
    ...
    for (const row of rows) { if (!row.valid) { result.invalid += 1; continue; } const existing = await c.users.findOne({ schoolId: actor.schoolId!, $or: [{ email: row.email }, { username: row.username }] }); const now = new Date(); if (existing) { if (existing.role !== "STUDENT" || existing.teacherId !== actor.id) { result.duplicates += 1; continue; } await c.users.updateOne({ id: existing.id }, { $set: { name: row.name, updatedAt: now } }); await c.studentProfiles.updateOne({ studentUserId: existing.id, teacherId: actor.id, schoolId: actor.schoolId! }, { $set: { studentId: row.studentId || null, usn: row.usn || null, branch: row.branch || null, semester: row.semester || null, section: row.section || null, className: row.className || null, updatedAt: now } }); result.updated += 1; continue; }
      const user: LmsUser = { id: await nextId(db, "users"), openId: null, name: row.name, email: row.email, loginMethod: "credentials", username: row.username, passwordHash: await bcrypt.hash(input.defaultTemporaryPassword, 12), role: "STUDENT", schoolId: actor.schoolId!, teacherId: actor.id, status: "ACTIVE", lastLogin: null, createdAt: now, updatedAt: now, lastSignedIn: now }; const profile: StudentProfile = { id: await nextId(db, "studentProfiles"), studentUserId: user.id, schoolId: actor.schoolId!, teacherId: actor.id, studentId: row.studentId || null, usn: row.usn || null, branch: row.branch || null, semester: row.semester || null, section: row.section || null, className: row.className || null, createdAt: now, updatedAt: now }; await c.users.insertOne(user); await c.studentProfiles.insertOne(profile); result.created += 1;
    }
