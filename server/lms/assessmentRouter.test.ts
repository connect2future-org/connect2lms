...
import { selectImportedStudentIds, selectPublishStudentIds } from "./assessmentRouter";
...
describe("commercial institution assessment policy helpers", () => {
  ...
  it("normalizes imported publish targets and rejects empty selected targets", () => { expect(selectPublishStudentIds("ALL_ACTIVE", [11, 11, 12])).toEqual([11, 12]); expect(selectPublishStudentIds("SELECTED_ACTIVE", [11, 12, 13], [12, 12])).toEqual([12]); expect(selectImportedStudentIds("IMPORTED_ALL", [11, 11, 12])).toEqual([11, 12]); expect(selectImportedStudentIds("IMPORTED_SELECTED", [], [12, 12, 13])).toEqual([12, 13]); expect(() => selectPublishStudentIds("SELECTED_ACTIVE", [11, 12], [])).toThrow("SELECT_AT_LEAST_ONE_STUDENT"); expect(() => selectImportedStudentIds("IMPORTED_SELECTED", [], [])).toThrow("SELECT_AT_LEAST_ONE_STUDENT"); });
});
